...
cp "$TEMPLATE_CONF" "$KEYDB_CONFIG"

# Second pass: Replace the variables in $KEYDB_CONFIG
while IFS='=' read -r key value || [[ -n $key ]]; do
    # Skip comments and empty lines
    [[ $key =~ ^#.* ]] || [[ -z $key ]] && continue
    ...
    # Replace occurrences of the key with the value in $KEYDB_CONFIG
    sed -i "s/\${$key}/${value_escaped}/g" "$KEYDB_CONFIG"
done < .env

echo "Configuration file is generated at $KEYDB_CONFIG"
